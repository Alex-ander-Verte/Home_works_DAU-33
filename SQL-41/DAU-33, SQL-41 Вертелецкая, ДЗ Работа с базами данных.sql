...
from film
where (rating::text like 'R' and (rental_rate between 0.00 and 3.00)) or (rating::text like 'PG-13' and rental_rate >= 4.00);
...
select customer_id , email , upper(left(split_part(email, '@', 1), 1))||lower(substring(split_part(email, '@', 1), 2, character_length(split_part(email, '@', 1))))  as "Email before @", upper(left(split_part(email, '@', 2), 1))||lower(substring(split_part(email, '@', 2), 2, character_length(split_part(email, '@', 2)))) as "Email after @"
from customer;
